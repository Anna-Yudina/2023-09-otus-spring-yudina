...
class BookRepositoryJdbcTest {
    ...
    @DisplayName("должен загружать список книг по id автора")
    @ParameterizedTest
    @MethodSource("getDbAuthors")
    void shouldReturnCorrectBooksListByAuthorId(Author author){
        var actualBooks = repositoryJdbc.findByAuthorId(author.getId());
        var expectedBooks = dbBooks.stream()
                .filter(book -> book.getAuthor().getId() == author.getId()).collect(Collectors.toList());
